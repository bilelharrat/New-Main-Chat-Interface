    ...
    return (
      <div className="h-full bg-white dark:bg-gray-900 text-black dark:text-white flex flex-col w-16 items-center nav-spacing py-3 transition-all duration-300 shadow-apple">
        <div className="flex flex-col items-center nav-spacing w-full flex-1">
          ...
          >
            <Search size={18} className="text-apple-secondary" />
          </button>
          <button
            className="flex items-center justify-center w-10 h-10 rounded-xl transition hover:bg-gradient-to-r hover:from-apple-success hover:to-apple-primary hover:text-white text-apple-success m-sm focus-ring"
            title="New Chat"
      ...
      >
          ...
          <div className="flex flex-col component-gap-xs px-2">
            <div className="flex items-center component-gap-md bg-white dark:bg-gray-900 px-3 min-h-12">
              <div
                ...
                onClick={() => setCurrentView('prompt-console')}
              >
                <div className="text-white flex items-center justify-center rounded-lg bg-gradient-to-r from-blue-500 to-purple-600 shrink-0 size-8 shadow-sm">
                  <Plus size={18} />
                </div>
              ...
              >
                <div
                  className={`flex items-center w-full component-gap-md cursor-pointer transition-all duration-300 rounded-xl px-3 py-2.5 focus-ring-enhanced ${
                    currentView === item.id 
                      ? 'bg-gradient-to-r from-blue-500 to-purple-600 text-white shadow-apple-medium transform scale-105' 
        ...
        <div
          ...
          className={`absolute right-0 top-0 bottom-0 w-2 cursor-col-resize transition-all duration-200 group ${
            isResizing ? 'bg-apple-success' : 'hover:bg-apple-success/30'
          }`}
        ...
        >
          ...
          <div className="absolute right-0 top-0 bottom-0 w-0.5 bg-apple-success/20 group-hover:bg-apple-success/40 transition-colors duration-200"></div>
          <div className={`absolute right-0.5 top-1/2 -translate-y-1/2 w-1 h-12 rounded-full transition-all duration-200 ${
            isResizing ? 'bg-white opacity-100' : 'bg-apple-success/50 opacity-0 group-hover:opacity-100'
          }`}></div>
          {isResizing && (
            <div className="absolute right-0 top-0 bottom-0 w-0.5 bg-apple-success"></div>
          )}
